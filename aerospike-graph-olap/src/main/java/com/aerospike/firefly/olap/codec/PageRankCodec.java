...
import static com.aerospike.firefly.olap.process.PageRankProgram.*;
...
public class PageRankCodec implements Codec {
    ...
    public static List<String> getOutVertexIds(final Vertex vertex) {
        if (vertex instanceof DetachedVertex) {
            return (List<String>) vertex.property(OUT_VERTICES).value();
        }
        final List<FireflyId> cachedIds = IteratorUtils.asList(
                ((FireflyVertex) vertex).getVertexIdsFromVertex(Direction.OUT, Collections.emptySet()));

        final List<String> outVertexIds = new ArrayList<>(cachedIds.size());
        cachedIds.forEach(id -> outVertexIds.add(id.toString()));
        return outVertexIds;
    }
}
