...
public class ValidateLambdaCodeInjectionDisabledTest extends AbstractFireflySuite {
    ...
    public void testValidateLambdaCodeInjectionDisabled() {
        ...
        try {
            final List<Vertex> markos = g.V().has("name", "marko").filter(Lambda.predicate("it.get().label().equals('person')")).toList();
        ...
        try {
            final List<Object> markoOuts = g.V().has("name", "marko").flatMap(Lambda.function("it.get().vertices(Direction.OUT)", "gremlin-groovy")).toList();
            fail("Lambda code injection should be disabled.");
        } catch (final Exception ignored) {
            // Expect an exception.
        }
