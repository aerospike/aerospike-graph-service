...
public class TestBulkLoaderCallEntryPoint {
    ...
    public void validateInputDataTrue() {
        ...
        try (final FireflyGraph fireflyGraph = FireflyGraph.open(config)) {
            ...
            Assert.assertEquals(0, g.V().count().next().longValue());
            Assert.assertEquals(0, g.E().count().next().longValue());
            g.call("bulk-load").with("aerospike.graphloader.config", "src/test/resources/conf/packed/config.properties").with("validate-input-data", true).iterate();
            Assert.assertNotEquals(0, g.V().count().next().longValue());
            Assert.assertNotEquals(0, g.E().count().next().longValue());
        }
    }
    ...
    public void validateInputDataFalse() {
        ...
        try (final FireflyGraph fireflyGraph = FireflyGraph.open(config)) {
            ...
            Assert.assertEquals(0, g.V().count().next().longValue());
            Assert.assertEquals(0, g.E().count().next().longValue());
            g.call("bulk-load").with("aerospike.graphloader.config", "src/test/resources/conf/packed/config.properties").with("validate-input-data", false).iterate();
            Assert.assertNotEquals(0, g.V().count().next().longValue());
            Assert.assertNotEquals(0, g.E().count().next().longValue());
        }
    }
    ...
    public void validateInputDataInvalidInput() {
        ...
        try (final FireflyGraph fireflyGraph = FireflyGraph.open(config)) {
            ...
            Assert.assertEquals(0, g.V().count().next().longValue());
            Assert.assertEquals(0, g.E().count().next().longValue());
            g.call("bulk-load").with("aerospike.graphloader.config", "src/test/resources/conf/packed/config.properties").with("validate-input-data", "notABoolean").iterate();
            Assert.fail("Expected call to fail.");
        } catch (final Exception e) {
            Assert.assertEquals("Expected bulk loader flag 'validate-input-data' to be set to a boolean value. Instead value was set with type 'java.lang.String'.", e.getMessage());
        }
    }
