...
public class DataModelLeakageTest {
    ...
    public void testOverwritingVertexProperty() {
        ...
        final Key recordKey = new Key(SETUP_GRAPH.getBaseGraph().namespace, SETUP_GRAPH.getBaseGraph().VERTEX_AERO_SET,
                Value.get(id));
        ...
        var vpBin = record.getMap(SETUP_GRAPH.getBaseGraph().VERTEX_PROPERTY_DATA_BIN);
        var vpThBin = record.getMap(SETUP_GRAPH.getBaseGraph().VERTEX_PROPERTY_TH_BIN);
        var vppBin = record.getMap(SETUP_GRAPH.getBaseGraph().VP_PROPERTY_BIN);
        ...
        vpBin = record.getMap(SETUP_GRAPH.getBaseGraph().VERTEX_PROPERTY_DATA_BIN);
        vpThBin = record.getMap(SETUP_GRAPH.getBaseGraph().VERTEX_PROPERTY_TH_BIN);
        vppBin = record.getMap(SETUP_GRAPH.getBaseGraph().VP_PROPERTY_BIN);
    ...
    public void testRemovingFromEdgeCache() {
        ...
        final Key fromRecordKey = new Key(SETUP_GRAPH.getBaseGraph().namespace,
                SETUP_GRAPH.getBaseGraph().VERTEX_AERO_SET, Value.get(fromId));
        final Key toRecordKey = new Key(SETUP_GRAPH.getBaseGraph().namespace,
                SETUP_GRAPH.getBaseGraph().VERTEX_AERO_SET, Value.get(toId));
        for (int i = 0; i < 2; i++) {
        ...
        var outBin = fromRecord.getMap(SETUP_GRAPH.getBaseGraph().OUT_EDGES_BIN);
        var inBin = toRecord.getMap(SETUP_GRAPH.getBaseGraph().IN_EDGES_BIN);
        ...
        outBin = fromRecord.getMap(SETUP_GRAPH.getBaseGraph().OUT_EDGES_BIN);
        inBin = toRecord.getMap(SETUP_GRAPH.getBaseGraph().IN_EDGES_BIN);
        Assert.assertEquals(2, outBin.size());
        Assert.assertEquals(2, inBin.size());
    ...
    public void testOverwritingEdgeProperty() {
        ...
        final Key key = new Key(SETUP_GRAPH.getBaseGraph().namespace,
                SETUP_GRAPH.getBaseGraph().EDGE_AERO_SET, Value.get(recordId));

        Record record = SETUP_GRAPH.getBaseGraph().read(key, null);
        var phatEdgeDataMap = record.getMap(SETUP_GRAPH.getBaseGraph().EDGE_DATA_BIN);
        ...
        phatEdgeDataMap = record.getMap(SETUP_GRAPH.getBaseGraph().EDGE_DATA_BIN);
    ...
    public void testOverWritingSupernodeEdgeProperty() {
        ...
        try (final FireflyGraph graph = FireflyGraph.open(config)) {
            ...
            final Key key = new Key(graph.getBaseGraph().namespace,
                    graph.getBaseGraph().EDGE_AERO_SET, Value.get(recordId));

            Record record = graph.getBaseGraph().read(key, null);
            var phatEdgeDataMap = record.getMap(graph.getBaseGraph().EDGE_DATA_BIN);
            Map<Long, Long> edgeThMap = (Map<Long, Long>) phatEdgeDataMap.values().iterator().next();
            Map<Long, Object> edgePropertiesOutMap = (Map<Long, Object>) (record.getMap(graph.getBaseGraph().SUPERNODES_OUT_BIN).values().iterator().next());
            Map<Long, Object> edgePropertiesInMap = (Map<Long, Object>) (record.getMap(graph.getBaseGraph().SUPERNODES_IN_BIN).values().iterator().next());
            Assert.assertEquals(2, edgeThMap.size());
            ...
            phatEdgeDataMap = record.getMap(graph.getBaseGraph().EDGE_DATA_BIN);
            edgeThMap = (Map<Long, Long>) phatEdgeDataMap.values().iterator().next();
            edgePropertiesOutMap = (Map<Long, Object>) (record.getMap(graph.getBaseGraph().SUPERNODES_OUT_BIN).values().iterator().next());
            edgePropertiesInMap = (Map<Long, Object>) (record.getMap(graph.getBaseGraph().SUPERNODES_IN_BIN).values().iterator().next());
            Assert.assertEquals(1, edgeThMap.size());
            // These bins contain 2 extra values - label and adjacent vertex id
            Assert.assertEquals(4, edgePropertiesOutMap.size());
            Assert.assertEquals(4, edgePropertiesInMap.size());
