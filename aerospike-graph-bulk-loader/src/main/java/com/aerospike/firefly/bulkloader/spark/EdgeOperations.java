...
public class EdgeOperations implements Serializable {
     ...
     * @return encoded
     */
    private static String encodeID(byte[] arr) {

        //isolate bytes from both component
        byte[] recycleBytes = new byte[8];
        byte[] numericBytes = new byte[8];
        ...
        ByteBuffer bb = ByteBuffer.wrap(recycleBytes);
        Long recycleId = bb.getLong();
        ByteBuffer bb1 = ByteBuffer.wrap(numericBytes);
        Long numericID = bb1.getLong();
