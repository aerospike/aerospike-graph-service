...
public class SindexServiceCreate<I, R> extends SindexServiceBase<I, R> {
    ...
    protected String usage(final Map params) {
        return String.format("Illegal arguments provided to %s.\n" +
                        ...
                        "\t\tg.call(\"aerospike.graph.admin.index.create\").with(\"" + ELEMENT_TYPE + "\", \"vertex\").with(\"" + PROPERTY_KEY + "\", \"~label\").next();\n" +
                        "\t\tg.call(\"aerospike.graph.admin.index.create\").with(\"" + ELEMENT_TYPE + "\", \"vertex\").with(\"" + PROPERTY_KEY + "\", \"name\").next();",
                getName(), params);
    }
    ...
    protected R execute(final Map params) {
        if (params.get(ELEMENT_TYPE).equals("vertex")) {
            if (params.get(PROPERTY_KEY).equals("~label")) {
                return (R) Admin.index.createVertexLabelIndex(firefly, new EmptyAdminContext());
            } else {
                return (R) Admin.index.createVertexPropertyIndex(firefly, (String) params.get(PROPERTY_KEY), new EmptyAdminContext());
            }
        } else {
