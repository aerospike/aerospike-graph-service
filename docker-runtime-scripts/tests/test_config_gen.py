...
def test_main_scan_disabled_by_default(tmp_path, monkeypatch):
    """Test that aerospike.graph.scan.enabled defaults to false"""
    input_properties = tmp_path / "input.properties"
    default_yaml = tmp_path / "default.yaml"
    output_yaml = tmp_path / "out.yaml"
    conf_dir = tmp_path / "conf"
    ...
    default_yaml.write_text("host: localhost\n")
    input_properties.write_text("aerospike.namespace=test\n")
    ...
    graph_props = conf_dir / "aerospike-graph-graph.properties"
    content = graph_props.read_text()
    assert "aerospike.graph.scan.enabled=false" in content


def test_main_scan_enabled_global_override(tmp_path, monkeypatch):
    """Test that aerospike.graph.scan.enabled can be overridden globally"""
    input_properties = tmp_path / "input.properties"
    default_yaml = tmp_path / "default.yaml"
    output_yaml = tmp_path / "out.yaml"
    conf_dir = tmp_path / "conf"
    ...
    input_properties.write_text(textwrap.dedent("""\
        aerospike.graph-service.graphs=graph1,graph2
        aerospike.namespace=test
        aerospike.graph.scan.enabled=true
    """))
    ...
    # Both graphs should have scan enabled
    graph1_props = conf_dir / "aerospike-graph-graph1.properties"
    graph2_props = conf_dir / "aerospike-graph-graph2.properties"

    assert "aerospike.graph.scan.enabled=true" in graph1_props.read_text()
    assert "aerospike.graph.scan.enabled=true" in graph2_props.read_text()


def test_main_scan_enabled_per_graph_override(tmp_path, monkeypatch):
    """Test that aerospike.graph.scan.enabled can be overridden per-graph"""
    input_properties = tmp_path / "input.properties"
    default_yaml = tmp_path / "default.yaml"
    output_yaml = tmp_path / "out.yaml"
    conf_dir = tmp_path / "conf"
    ...
    input_properties.write_text(textwrap.dedent("""\
        aerospike.graph-service.graphs=graph1,graph2
        aerospike.namespace=test
        graph1.aerospike.graph.scan.enabled=true
    """))
    ...
    # graph1 should have scan enabled, graph2 should have default (false)
    graph1_props = conf_dir / "aerospike-graph-graph1.properties"
    graph2_props = conf_dir / "aerospike-graph-graph2.properties"
    ...
    assert "aerospike.graph.scan.enabled=true" in graph1_content
    assert "aerospike.graph.scan.enabled=false" in graph2_content