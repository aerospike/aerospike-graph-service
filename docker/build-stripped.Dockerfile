...
RUN cd /tmp
RUN curl -L -o maven.tar.gz $MAVEN_URL
RUN curl -L -o gremlin-console.zip $GREMLIN_CONSOLE_URL
RUN curl -L -o gremlin-server.zip $GREMLIN_SERVER_URL
RUN curl -L -o jansi-$JANSI_VERSION.jar $JANSI_URL
RUN tar -zxvf maven.tar.gz -C /opt/
RUN unzip -qq gremlin-console.zip -d /opt/ && ln -sf /opt/apache-tinkerpop-gremlin-console-$TINKERPOP_VERSION /opt/gremlin-console
RUN unzip -qq gremlin-server.zip -d /opt/ && ln -sf /opt/apache-tinkerpop-gremlin-server-$TINKERPOP_VERSION /opt/gremlin-server
RUN mv jansi-$JANSI_VERSION.jar /opt/gremlin-console/lib
RUN rm -rf gremlin-console.zip
RUN rm -rf gremlin-server.zip
