...
public class FireflySummaryUpdater implements Closeable {
    ...
    private final AtomicBoolean exitedWithError = new AtomicBoolean(false);
    ...
    public boolean exitedWithError() {
        return this.exitedWithError.get();
    }
    ...
    public static class LabelCountInfo implements UpdateInfo {
        ...
        @Override
        public String toString() {
            return "VertexInfo{label='" + label + "',count=" + count + "}";
        }
    }
    ...
    public FireflyElementMetadata getFireflyStatistics() {
        ...
        final Record vertexInfo = db.getClient().get(null, new Key(db.getNamespace(), db.SUMMARY_SET, V_SUMMARY_RECORD));
        final Map<String, FireflyPropertiesAndCount> vertexMetadata = new HashMap<>();
        if (vertexInfo != null) {
            for (final String vertexLabel : vertexInfo.bins.keySet()) {
                final Record propertyInfo = db.getClient().get(null, new Key(db.getNamespace(), db.SUMMARY_SET, VP_PROPERTY_PREFIX + vertexLabel));
                if (propertyInfo != null) {
                    vertexMetadata.put(vertexLabel, new FireflyPropertiesAndCount(propertyInfo.bins.keySet(), (long) vertexInfo.bins.get(vertexLabel)));
                } else {
                    vertexMetadata.put(vertexLabel, new FireflyPropertiesAndCount(Set.of(), (long) vertexInfo.bins.get(vertexLabel)));
                }
            }
        }

        // Grab edge metadata.
        final Record edgeInfo = db.getClient().get(null, new Key(db.getNamespace(), db.SUMMARY_SET, E_SUMMARY_RECORD));
        final Map<String, FireflyPropertiesAndCount> edgeMetadata = new HashMap<>();
        if (edgeInfo != null) {
            for (final String edgeLabel : edgeInfo.bins.keySet()) {
                final Record propertyInfo = db.getClient().get(null, new Key(db.getNamespace(), db.SUMMARY_SET, EP_PROPERTY_PREFIX + edgeLabel));
                if (propertyInfo != null) {
                    edgeMetadata.put(edgeLabel, new FireflyPropertiesAndCount(propertyInfo.bins.keySet(), (long) edgeInfo.bins.get(edgeLabel)));
                } else {
                    edgeMetadata.put(edgeLabel, new FireflyPropertiesAndCount(Set.of(), (long) edgeInfo.bins.get(edgeLabel)));
                }
            }
        }
        ...
        return () -> {
            ...
            while (true) {
                ...
                try {
                    ...
                    writeLabelCountOperations(new HashSet<>(vertexUpdates.values()), V_SUMMARY_RECORD);
                    writeLabelCountOperations(new HashSet<>(edgeUpdates.values()), E_SUMMARY_RECORD);
                    writeLabelPropertiesOperations(new HashSet<>(vertexUpdates.values()), vertexLabelToProperties, VP_PROPERTY_PREFIX);
                    writeLabelPropertiesOperations(new HashSet<>(edgeUpdates.values()), edgeLabelToProperties, EP_PROPERTY_PREFIX);

                    // Write the data to the summary record.
                    if (foundPoisonPill) {
                        LOG.info("Taking poison pill.");
                        break;
                    }
                } catch (final Exception e) {
                    LOG.error("Encountered fatal error while updating summary. Exiting summary thread.", e);
                    exitedWithError.set(true);
                    break;
                }
            }
    ...
    private void writeLabelCountOperations(final Set<LabelCountInfo> updates, final String summaryRecordKey) {
        ...
        for (final LabelCountInfo countInfo : updates) {
            ...
            // UPDATE_ONLY operation is to add the count to the existing count.
            // CREATE_ONLY operation is to create the bin with count.
            Operation update = ExpOperation.write(countInfo.label, Exp.build(Exp.add(Exp.intBin(countInfo.label), Exp.val(countInfo.count))), ExpWriteFlags.UPDATE_ONLY | ExpWriteFlags.POLICY_NO_FAIL);
            Operation create = ExpOperation.write(countInfo.label, Exp.build(Exp.val(countInfo.count)), ExpWriteFlags.CREATE_ONLY | ExpWriteFlags.POLICY_NO_FAIL);
            db.getClient().operate(writePolicy, new Key(db.getNamespace(), db.SUMMARY_SET, summaryRecordKey), update, create);
        }
    }

    private void writeLabelPropertiesOperations(final Set<LabelCountInfo> updates, final Map<String, Set<String>> propertyMappings, final String prefix) {
        // Standard write policy.
        final WritePolicy writePolicy = new WritePolicy();
        ...
        for (final LabelCountInfo updateInfo : updates) {
            for (final String property : updateInfo.properties) {
                ...
                // CREATE_ONLY operation is to create the bin with a junk value in. Presence of bin is all we need.
                Operation create = ExpOperation.write(property, Exp.build(Exp.val(0)), ExpWriteFlags.CREATE_ONLY | ExpWriteFlags.POLICY_NO_FAIL);
                db.getClient().operate(writePolicy, new Key(db.getNamespace(), db.SUMMARY_SET, prefix + updateInfo.label), create);
            }
        }
    }
}
