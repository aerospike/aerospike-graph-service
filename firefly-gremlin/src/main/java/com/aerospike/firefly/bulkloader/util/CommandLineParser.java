...
public class CommandLineParser {
    ...
    static public CommandLine parseCmdArgs(final String[] args) {
        ...
        final Option fsOption = new Option("f", "filesystem", true, "FileSystem storage type. If non specified uses local, else 's3'");
        options.addOption(fsOption);

        final Option usernameOption = new Option("u", "username", true, "Username/ID credential for cloud storage.");
        options.addOption(usernameOption);

        final Option passKeyOption = new Option("p", "passkey", true, "Password/Key/Secret credential for cloud storage.");
