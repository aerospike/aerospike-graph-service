...
RUN mkdir /opt/bulk-loader &&\
    mv /opt/aerospike-graph/aerospike-graph-bulk-loader/target/aerospike-graph-bulk-loader-2.0.0.jar /opt/bulk-loader

# Move sizing-tool jar to /opt/sizing-tool.
RUN mkdir /opt/sizing-tool &&\
    mv /opt/aerospike-graph/aerospike-graph-sizing-tool/target/aerospike-graph-sizing-tool-2.0.0.jar /opt/sizing-tool
...
RUN \
    ...
    else gremlin-server.sh install 'com.aerospike aerospike-graph-gremlin 2.0.0' ;  \
    fi
