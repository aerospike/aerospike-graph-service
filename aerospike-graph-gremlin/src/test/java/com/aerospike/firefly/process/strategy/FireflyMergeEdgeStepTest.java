...
public class FireflyMergeEdgeStepTest {
    ...
    public void testConcurrentWriting() throws InterruptedException {
        ...
        try {
            ...
            concurrentWritingTest(g, graph.traversal(), graph.traversal());
        } finally {
            graph.getBaseGraph().dropDatabase(graph, false);
            graph.close();
        }
    ...
    static public void concurrentWritingTest(final GraphTraversalSource g, final GraphTraversalSource g2,
                                      final GraphTraversalSource g3) throws InterruptedException {
        ...
        for (int i = 0; i < 3; i++) {
            final Thread task = new Thread(() -> {
                final GraphTraversalSource threadG = g2;
                for (int j = 0; j < 100; j++) {
        ...
        for (int i = 0; i < 3; i++) {
            final Thread task = new Thread(() -> {
                final GraphTraversalSource threadG = g3;
                for (int j = 0; j < 100; j++) {
                    final Map<Object, Object> mergeMap = new HashMap<>();
    ...
    static public void concurrentWithOptionsTest(final GraphTraversalSource g) throws InterruptedException {
